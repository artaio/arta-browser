...
const groupByDate = (
  data: ArtaPackageEvent[]
): Record<string, ArtaPackageEvent[]> => {
...
export const PackageEvents = ({
  ...
  const [eventHistory, setEventHistory] =
    useState<Array<ArtaPackageEvent> | null>(null);
  const [groupedEventHistory, setGroupedEventHistory] = useState<Record<
    string,
    ArtaPackageEvent[]
  > | null>(null);
  ...
  const pkgTracking = shipment.tracking.find(
    (t) => t.package_id === shipment.packages[packageIdx].id
  );
    ...
    <div class="artajs__tracking__events__wrapper">
      ...
      <div class="artajs__tracking__events__header">
        <div class="artajs__tracking__events__header__title">
          {config.text.packageLabel} #{packageIdx + 1}{' '}
          {config.text.historyLabel}
        </div>
        {pkgTracking?.carrier_name &&
      ...
      {groupedEventHistory ? (
        <div class="artajs__tracking__events__body">
          {Object.values(groupedEventHistory).map((events, idx) => {
            return (
              <div class="artajs__tracking__events__group">
                ...
                <div class="artajs__tracking__events__group__item">
                  {events.map((event, idx2) => {
                    return (
                      <div class="artajs__tracking__events__group__content">
                        <div
                          class={
                            idx === 0 && idx2 === 0
                        ...
                        <div class="artajs__tracking__events__group__summary">
                          ...
                          >
                            {cleanLocationString(event.location) ??
                              config.text.unknownLocation}
                          </div>
